...
\tcode{ranges::iter_move(E)} for some expression \tcode{E} is equivalent to
\tcode{iter_move(E)}, with overload resolution~(\cxxref{over.match}) performed
in a context described below.

\pnum
The context in which the expression \tcode{iter_move(E)} is evaluated
\begin{itemize}
\item does not include a declaration of \tcode{ranges::iter_move},
\item includes the lookup set produced by argument-dependent
  lookup~(\cxxref{basic.lookup.argdep}), and
\item includes an \tcode{iter_move} function template equivalent to the
  following:
\begin{itemdecl}
// \expos
...
\begin{itemdescr}
\pnum
The return type is \tcode{Ret} where \tcode{Ret} is
\tcode{remove_reference_t<reference_t<I>{}>\&\&} if
\tcode{reference_t<I>} is a reference type;
\tcode{decay_t<reference_t<I>{}>}, otherwise.

\pnum
The expression in the \tcode{noexcept} is equivalent to:
\begin{codeblock}
noexcept(static_cast<Ret>(*r))
\end{codeblock}

\pnum
\effects Equivalent to: \tcode{return static_cast<Ret>(*r);}
\end{itemdescr}
\end{itemize}

\pnum
If, for the \tcode{iter_move} function selected by overload resolution,
...
is equivalent to \tcode{(void)iter_swap(E1, E2)}, with overload
resolution~(\cxxref{over.match}) performed in a context described below:

\pnum
The context in which the expression \tcode{(void)iter_swap(E1, E2)} is evaluated

\begin{itemize}
\item does not include a declaration of \tcode{ranges::iter_swap},
\item includes the lookup set produced by argument-dependent
  lookup~(\cxxref{basic.lookup.argdep}), and
\item includes \tcode{iter_swap} function template(s) equivalent to the
  following:
\begin{itemdecl}
// \expos
template <class I1, class I2>
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{ranges::swap(*r1, *r2)}.
\end{itemdescr}
...
\end{itemdescr}
\pnum
\effects Exchanges values referred to by two \tcode{Readable} objects.

\pnum
\enterexample Below is a possible implementation:
\begin{codeblock}
...
\pnum
If the \tcode{iter_swap} function selected by overload resolution does not swap

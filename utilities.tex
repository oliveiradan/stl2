...
@\newtxt{namespace std \{}@
  ...
  template<class T>
    @\added{requires \newtxt{experimental::ranges_v1::}MoveConstructible<T>\newtxt{()} \&\&}@
      @\added{\newtxt{experimental::ranges_v1::}MoveAssignable<T>\newtxt{()} \&\& \newtxt{experimental::ranges_v1::}Destructible<T>\newtxt{()}}@
...
\ednote{Change the \tcode{<experimental/ranges_v1/utility>} synopsis as follows:}
...
\ednote{To the end of the \tcode{<\newtxt{experimental/ranges_v1/}utility>} synopsis, add the following:}
...
\pnum
\remark
A library implementor is free to remove the \tcode{requires} clause so long as
...
b = move(tmp);
\end{codeblock}
shall have the effect of exchanging the values stored in the two locations.
\enternote For example, objects \tcode{a} and \tcode{b} may not be reference-like
types like \tcode{reference_wrapper}~(\cxxref{refwrap}) that alias each other.\exitnote
}
